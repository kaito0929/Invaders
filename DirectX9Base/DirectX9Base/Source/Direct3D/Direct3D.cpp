...
bool Direct3D::Create(HWND hWnd)
{
	...
	if (FAILED(pDevice3D->CreateVertexBuffer(10000 * sizeof(VERTEX), D3DUSAGE_WRITEONLY, VERTEX3_FVF, D3DPOOL_MANAGED, &pointBuffer, nullptr)))
	{
...
HRESULT Direct3D::ClearScreen()//バックバッファのクリア
{
	if (isDeviceCreated)
	{
		DWORD ClearColor = 0x00000000;//背景クリア色 //赤　0xAARRGGBB
...
void Direct3D::DrawLine(VERTEX* pVertex, int count)
{
	//現在設定されている行列をリセット
	...
	pDevice3D->DrawPrimitiveUP(D3DPT_LINELIST, count, pVertex, sizeof(VERTEX));


}

void Direct3D::DrawPointSprite(VERTEX point[], int numPoint, Texture& texture)
{
	VERTEX *vertex;
	//インスタンスの取得
	Direct3D *pD3D = Direct3D::GetInstance();
	...
	pD3D->pDevice3D->SetTransform(D3DTS_WORLD, &m_world);

	//ポイントスプライトの描画
	pD3D->pDevice3D->SetRenderState(D3DRS_ZENABLE, false);	//Z比較を行わない
	pD3D->pDevice3D->SetStreamSource(0, pointBuffer, 0, sizeof(VERTEX));
	pD3D->pDevice3D->SetFVF(VERTEX3_FVF);
	pD3D->pDevice3D->DrawPrimitive(D3DPT_POINTLIST, 0, numPoint);
	pD3D->pDevice3D->SetRenderState(D3DRS_ZENABLE, true);	//Z比較を行う
